...
function chunkName(assets) {
  var chunkNamedAssets = {};
  _.each(assets, function(assetPath, assetFile) {
      chunk = assetFile.replace('.js', '');
      chunkNamedAssets[chunk] = assetPath;
  });
  return chunkNamedAssets;
};
...
ManifestPlugin.prototype.apply = function(compiler) {
  ...
  compiler.plugin('emit', function(compilation, compileCallback) {
    ...
    var json = JSON.stringify({ assetsByChunkName: chunkName(cache), errors: [] }, null, 2);
